...
**Per-equation term explainers added 2026-05-27.** Every display equation now has a "Where" legend (`<aside class="where">`) naming each symbol with its *type* (scalar / vector in ℝⁿ / matrix / distribution / function) and role, in the style the user asked for. Tightly-related equations are clustered under one legend (Ch3 Adam pair, Ch5 Q/K/V trio + block pair, Ch6 likelihood+CE, Ch10 DDPM forward/closed-form/loss and the CFG pair). Two prose-shaped "equations" (Ch8 `accuracy ≈ f(…)`, Ch2 logistic-sigmoid line) get an inline sentence instead of a heavy block, per advisor. Linear-algebra jargon is defined inline with `<dfn title="…">` (dotted-underline + hover tooltip): column space, orthogonal projection, hat matrix, pseudoinverse, rank-deficient, PSD, Gram matrix, Hilbert space, inner product, quadratic program, hinge loss (Ch2); cosine similarity (Ch9); stochastic differential equation (Ch10). Verification: all 778 math expressions in the chapters render under KaTeX `strict:"error"` (stricter than the book's runtime `strict:"ignore"`) with 0 failures — checked via a temp Node+KaTeX harness, since `throwOnError:false` would otherwise hide a broken formula as raw text. `aside.where dl` now stacks to one column at ≤600px so wide symbol cells don't overflow on mobile.
